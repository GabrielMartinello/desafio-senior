package com.desafio.senior.desafiosenior;
...
public class DesafioSeniorApplication {
	...
	public static void main(String[] args) {
		Logger logger = LoggerFactory.getLogger(DesafioSeniorApplication.class);
		...
		try {
			...
			statement.executeQuery("SELECT count(*) FROM pg_database WHERE datname = 'senior'");
			ResultSet resultSet = statement.getResultSet();
			resultSet.next();
			int count = resultSet.getInt(1);

			if (count <= 0) {
				statement.executeUpdate("CREATE DATABASE senior");
